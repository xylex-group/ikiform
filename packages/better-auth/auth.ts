...
function ensureDirectory(dirPath: string): boolean {
	try {
		...
		console.warn(
			`[athena-auth] Failed to create auth directory "${dirPath}". Falling back to another location.`,
			error
		);
		return false;
	}
...
function resolveAuthDatabasePath(): string {
	...
	if (explicitDatabasePath) {
		...
		if (!ensureDirectory(explicitDirectory)) {
			throw new Error(
				`ATHENA_AUTH_DB_PATH directory is not writable: ${explicitDirectory}`
			);
		}
	...
	throw new Error(
		"Unable to initialize Athena Auth SQLite storage. Set ATHENA_AUTH_DB_PATH or ATHENA_AUTH_DATA_DIR to a writable path."
	);
}
...
if (
	...
	throw new Error("ATHENA_AUTH_SECRET must be set in production.");
}
...
export const auth = betterAuth({
	...
	emailAndPassword: {
		...
		sendResetPassword: async ({ url, user }) => {
			console.info(
				`[athena-auth] Reset password link for ${user.email}: ${url}`
			);
		},
	},
	plugins: [
		organization({
			async sendInvitationEmail(data) {
				...
				console.info(
					`[athena-auth] Invitation link for ${data.email}: ${inviteLink}`
				);
			},
		}),
	],
