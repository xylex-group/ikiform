...
export interface PayloadFormsPagesFieldsRow {
  ...
  _parent_id: string
  field_id: string
  field_type: 'text' | 'textarea' | 'number' | 'email' | 'tel' | 'date' | 'select' | 'radio' | 'checkbox' | 'file' | 'password'
  label?: string | null
...
export type PayloadFormsPagesFieldsUpdate = Partial<PayloadFormsPagesFieldsInsert>

export const payloadFormsPagesFieldsModel = defineModel<PayloadFormsPagesFieldsRow, PayloadFormsPagesFieldsInsert, PayloadFormsPagesFieldsUpdate>({
  meta: {
    database: 'railway',
    schema: 'payload',
